...
import requests
from configuration.configuration import API_URL, BODY_STAND, API_URL_POST_ID, BODY_PUT, API_URL_COMMENT, BODY_PUT_COMMENT, API_URL_COMMENT_ID
...
def test_put_post(): 
    ...
    r = requests.put(API_URL_POST_ID + str(post_id), json=BODY_PUT, auth = ("admin",'123')) # редактирование поста. Тут в теле ответа будет "успешно"
    ...
    b = 0
    c = bodya.values()
    for i in BODY_PUT.values(): #проверка, что изменения внеслись в пост
        if i in c:
            b = b + 1
    assert b == len(BODY_PUT.values()), "Изменения записались неверно"
    print(bodya)
...
def test_post_delete(): 
    a =  requests.get(API_URL_POST_ID + str(post_id)) # отправка запроса на просмотр поста, с целью сохранения ег ов переменную
    bodya = a.json() # сохранение тела ответа комментария
    assert a.status_code == 200, "Неверный код ответа"
